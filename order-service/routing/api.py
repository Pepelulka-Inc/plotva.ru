...
async def get_user_orders(
    ...
    try:
        ...
        orders = await repositories.OrdersModel.get_user_orders(
            session,
            user_id
        )
...
async def update_order_status(
    ...
    async with session.begin():
        try:
            ...
            logger.debug(f"Current status: {order_obj.status}, new status: {data.new_status}")
            ...
            logger.critical(f"Unexpected error updating order {order_id}: {str(e)}", exc_info=True)
            await session.rollback()
            return api.SetStatusResponse(
...
async def delete_order(
    ...
    async with session.begin():
        try:
            ...
            await order_obj.delete_order(
                session,
                order_id
            )
            await session.commit()
            logger.info(f"Order {order_id} deleted successfully")
            return api.DeleteOrderResponse(
                error_message="",
                order_id=order_id
            )
