...
class BaseRequest(PydanticBaseModel):
    user_id: str = Field(default="123e4567-e89b-12d3-a456-426655440000")
...
class CreateOrderRequest(BaseRequest):
    ...
    order_time: datetime = Field(default=datetime.now())
    shipped_time: datetime = Field(default=datetime.now() + timedelta(days=1))
    address_id: str = Field(default="a6eebc99-9c0b-4ef8-bb6d-6bb9bd380a17")
...
class UpdateOrderRequest(BaseRequest):
    address_id: Optional[UUID] = Field(None, description="Новый адрес доставки")
    shipped_date: Optional[datetime] = Field(None, description="Новая дата доставки")
    status: Optional[OrderStatus] = Field(None, description="Новый статус заказа")
    entries: Optional[List[OrderEntryUpdate]] = Field(
        None, description="Список изменений позиций заказа"
    )
    note: Optional[str] = Field(
        None, max_length=500, description="Дополнительная заметка к заказу"
    )
...
class UpdateOrderResponse(BaseResponse):
    ...
    new_status: Optional[OrderStatus] = Field(
        None, description="Актуальный статус заказа после обновления"
    )
